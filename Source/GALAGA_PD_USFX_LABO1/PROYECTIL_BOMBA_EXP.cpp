...
APROYECTIL_BOMBA_EXP::APROYECTIL_BOMBA_EXP()
{
	static ConstructorHelpers::FObjectFinder<UStaticMesh>MeshAsset(TEXT("StaticMesh'/Game/ASSETS/VARIOS_A/Projectile.Projectile'"));
	if (MeshAsset.Succeeded())
	{
		Projectil_Mesh->SetStaticMesh(MeshAsset.Object);

		// Modificar la escala del componente de malla
		FVector NewScale(10.0f, 10.0f, 10.0f); // Escala modificada
	...
	Projectil_Collision->SetCapsuleHalfHeight(14.0f);
	Projectil_Collision->SetCapsuleRadius(14.0f);
}

void APROYECTIL_BOMBA_EXP::NotifyActorBeginOverlap(AActor* OtherActor)
{
